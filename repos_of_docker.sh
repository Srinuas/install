...
# ====== CONFIG ======
# మీ Docker Hub username & token ఇక్కడ పెట్టండి
export USERNAME="srinuas"           # <- మీ అసలు Docker Hub username
export TOKEN="docker token"         # <- Personal Access Token (PAT) ఇక్కడ పెట్టండి

if [[ -z "$USERNAME" || -z "$TOKEN" ]]; then
  echo "ERROR: USERNAME/TOKEN సెటప్ కాలేదు. దయచేసి స్క్రిప్ట్ టాప్‌లో సెట్ చేయండి."
  exit 1
fi
...
while [[ -n "$URL" && "$URL" != "null" ]]; do
  PAGE=$(curl -s -u "$USERNAME:$TOKEN" "$URL")
  if [[ -z "$PAGE" ]]; then
...
while [[ -n "$TAGS_URL" && "$TAGS_URL" != "null" ]]; do
  TPAGE=$(curl -s -u "$USERNAME:$TOKEN" "$TAGS_URL")
  if [[ -z "$TPAGE" ]]; then
...
# name, digest(short), last_updated, sizeMB
echo "$ALL_TAGS" | jq -r '
  .[] | {
    name: .name,
    digest: ( ( .images[0].digest // .digest // "" ) | sub("^sha256:";"") | .[0:12] ),
